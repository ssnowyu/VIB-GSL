...
class GraphLearner(nn.Module):
    def __init__(
        ...
        if metric_type == "attention":
            ...
            print(
                "[ Multi-perspective {} GraphLearner: {} ]".format(
                    metric_type, -num_pers
                )
            )
            ...
            self.linear_sims2 = nn.ModuleList(
                [
                    nn.Linear(self.hidden_size, hidden_size, bias=False)
                    for _ in range(num_pers)
                ]
            )
            print(
        ...
        if self.feature_denoise:
            self.feat_mask = self.construct_feat_mask(
                input_size, init_strategy="constant"
            )

        print(
            "[ Graph Learner metric type: {}, Graph Type: {} ]".format(
                metric_type, self.graph_type
            )
        )
    ...
    def reset_parameters(self):
        if self.feature_denoise:
            self.feat_mask = self.construct_feat_mask(
                self.input_size, init_strategy="constant"
            )
    ...
    def learn_adj(self, context, ctx_mask=None):
        ...
        if self.metric_type == "attention":
            ...
            for _ in range(self.num_pers):
                context_fc = torch.relu(
                    self.linear_sims2[_](torch.relu(self.linear_sims1[_](context)))
                )
                attention += torch.matmul(context_fc, context_fc.transpose(-1, -2))
        ...
        if self.graph_type == "epsilonNN":
            ...
            attention = self.build_knn_neighbourhood(
                attention, self.top_k, markoff_value
            )
        ...
        if self.graph_type in ["KNN", "epsilonNN"]:
            if self.metric_type in ("kernel", "weighted_cosine"):
                ...
                attention = attention / torch.clamp(
                    torch.sum(attention, dim=-1, keepdim=True), min=VERY_SMALL_NUMBER
                )
            elif self.metric_type == "cosine":
    ...
    def build_knn_neighbourhood(self, attention, top_k, markoff_value):
        ...
        weighted_adjacency_matrix = (
            markoff_value * torch.ones_like(attention)
    ...
    def build_prob_neighbourhood(self, attention, temperature=0.1):
        ...
        weighted_adjacency_matrix = RelaxedBernoulli(
            temperature=torch.Tensor([temperature]).to(attention.device),
            probs=attention,
        ).rsample()
        eps = 0.5
    ...
    def compute_distance_mat(self, X, weight=None):
        ...
        dists = (
            -2 * torch.matmul(trans_X, X.transpose(-1, -2))
            + norm.unsqueeze(0)
            + norm.unsqueeze(1)
        )
        return dists
    ...
    def mask_feature(self, x, use_sigmoid=True, marginalize=True):
        feat_mask = (
            torch.sigmoid(self.feat_mask) if use_sigmoid else self.feat_mask
        ).to(device)
        if marginalize:
