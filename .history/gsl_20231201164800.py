...
class VIBGSL(torch.nn.Module):
    def __init__(self, args, num_node_features, num_classes):
        ...
        if self.backbone == "GCN":
            self.backbone_gnn = myGCN(
                self.args,
                in_dim=self.num_node_features,
                out_dim=self.IB_size * 2,
                hidden_dim=self.hidden_dim,
            )
        elif self.backbone == "GIN":
            self.backbone_gnn = myGIN(
                self.args,
                in_dim=self.num_node_features,
                out_dim=self.IB_size * 2,
                hidden_dim=self.hidden_dim,
        ...
        self.graph_learner = GraphLearner(
            input_size=self.num_node_features,
            hidden_size=self.hidden_dim,
            graph_type=self.graph_type,
            top_k=self.top_k,
            epsilon=self.epsilon,
            num_pers=self.num_per,
            metric_type=self.graph_metric_type,
            feature_denoise=self.args.feature_denoise,
            device=None,
        )

        self.classifier = torch.nn.Sequential(
            Linear(self.IB_size, self.IB_size),
            ReLU(),
            Dropout(p=0.5),
    ...
    def forward(self, graphs):
        ...
        for graph in graphs_list:
            ...
            new_feature, new_adj = self.learn_graph(
                node_features=x,
            ...
            new_graph = Data(
                x=new_feature, edge_index=new_edge_index, edge_attr=new_edge_attr
            )
